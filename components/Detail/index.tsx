...
import Image from "next/image";
import { ExternalLink, History, Trophy, MoveRight, Menu, X } from "lucide-react";
import { useEffect, useState, use } from "react";  // ✅ FIX: import `use`
import { motion, AnimatePresence } from "framer-motion";
import Link from "next/link";
import Loading from "@/app/loading";
...
// ✅ FIX: params typed as Promise<{ slug: string }> for Next.js 14+ App Router
export default function Detail({ params }: { params: Promise<{ slug: string }> }) {

  // ✅ FIX: unwrap params with React `use()` — this gives you the real slug string
  ...
  useEffect(() => {
    ...
    const fetchHero = async () => {
      ...
      try {
        const res = await fetch(`${API_BASE}/api/hero/${slug}`);
        if (res.status === 404) { setError("Hero not found."); return; }
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.hero}>
        <Image src={`${API_BASE}/static/${hero.hero_image}`} fill alt={hero.name} className={styles.heroBg} sizes="100vw" priority />
        <div className={styles.heroGradient} />

        <header className={styles.heroHeader}>
          <nav className={styles.nav}>
            <button onClick={() => setNavOpen(!navOpen)} aria-label="Toggle navigation">
              {navOpen ? <X size={20} /> : <Menu size={20} />}
            </button>
            <ul className={styles.navList}>
              {NAV_LINKS.map((link) => (
          ...
          <AnimatePresence>
            {navOpen && (
              <motion.ul className={styles.mobileNav} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                {NAV_LINKS.map((link) => (
                  <li key={link.label}><a href={link.href} onClick={() => setNavOpen(false)}>{link.label}</a></li>
                ))}
              </motion.ul>
            )}
          </AnimatePresence>
        ...
        <div className={styles.heroContent}>
          ...
          {(hero.birth_year || hero.death_year) && (
            <div>
              <span>Active Period</span>
              <span>{hero.birth_year ?? "?"} – {hero.death_year ?? "present"}</span>
            </div>
          )}
        </div>
      </section>
        ...
        <div className={styles.detailGrid}>
          <main className={styles.mainCol}>
            ...
            <section className={styles.contentBox}>
              ...
              {comments.map((c, i) => (
                <div key={i} className={styles.comment}>
                  {c.avatar && (
                    <Image src={`${API_BASE}/static/${c.avatar}`} width={36} height={36} alt={c.full_name} style={{ borderRadius: "50%" }} />
                  )}
                  <div>
                    <strong>{c.full_name}</strong>
                    <span>{new Date(c.created_at).toLocaleDateString()}</span>
              ...
              <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Write a comment..." disabled={posting} />
          ...
          <aside className={styles.sidebar}>
            <div>
              ...
              {sources.map((s) => (
                <a key={s.id} href={s.source_link} target="_blank" rel="noreferrer">
                  <ExternalLink size={13} /> {s.source_title}
                </a>
              ))}
            </div>
          </aside>
